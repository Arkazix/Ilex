...
from src.const import ASSOCIATIVITY_LEFT, ASSOCIATIVITY_RIGHT
    ...
    def generate_token(self) -> Union[Stack, None]:
        ...
        while self.current_char != None:
            if self.current_char in WHITESPACE:
                ...
                self.advance()
                stack.push(Token(TokenType.POWER, 3, ASSOCIATIVITY_RIGHT))
            elif self.current_char == "!":
                self.advance()
                stack.push(Token(TokenType.FACTORIAL, 4, ASSOCIATIVITY_RIGHT))
            else:
                raise_error(
